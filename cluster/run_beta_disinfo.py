#!/usr/bin/env python3
"""
Run replicates for a single emission_temp (beta) value — disinformation pool.

Identical to run_beta.py except the claim pool contains 8 disinformation
posts at LLR = -1.0 (one-sided negative push).

Invoked by the Slurm array:
    python run_beta_disinfo.py <task_id> <output_dir>
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python run_beta_disinfo.py <task_id> <output_dir>")
        sys.exit(1)
