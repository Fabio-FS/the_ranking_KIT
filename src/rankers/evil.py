...
def rank(G, info, post_opinions, post_likes, post_seen_gen):
    ...
    for user_i in range(n_users):
        ...
        distances = np.abs(post_opinions[epsilon_authors, epsilon_times] - target)
        n_selected = min(k, n_within_epsilon)
        chosen = topk_closest(distances, n_within_epsilon, n_selected)
        selected_authors[user_i, :n_selected] = epsilon_authors[chosen]
        selected_times[user_i, :n_selected] = epsilon_times[chosen]
